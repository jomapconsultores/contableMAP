-- Aislamiento de la base `proyectos` dentro del PostgreSQL compartido.
--
...
--
-- Se ejecuta conectado a la base `proyectos`.

-- 1. Rol de conexión propio de Proyectos.
--    PostgREST se conecta con él y luego cambia a anon o service_role según
--    el token que traiga la petición.
do $$
begin
  if not exists (select 1 from pg_roles where rolname = 'authenticator_pr') then
    create role authenticator_pr with login noinherit;
  end if;
end $$;

alter role authenticator_pr with password :'clave';
...
grant anon, authenticated, service_role to authenticator_pr;
...
--    contra `proyectos`.
revoke connect on database proyectos from public;
grant  connect on database proyectos to authenticator_pr, postgres;

-- 3. Y que nadie cree objetos sueltos en el esquema public.
revoke create on schema public from public;

select 'aislamiento aplicado sobre proyectos' as resultado;
