-- Aislamiento de la base `atlas` dentro del PostgreSQL compartido.
--
...
--
-- Se ejecuta conectado a la base `atlas`.

-- 1. Rol de conexión propio de Atlas.
--    PostgREST se conecta con él y luego cambia a anon o service_role según
--    el token que traiga la petición.
do $$
begin
  if not exists (select 1 from pg_roles where rolname = 'authenticator_at') then
    create role authenticator_at with login noinherit;
  end if;
end $$;

alter role authenticator_at with password :'clave';
...
grant anon, authenticated, service_role to authenticator_at;
...
--    contra `atlas`.
revoke connect on database atlas from public;
grant  connect on database atlas to authenticator_at, postgres;

-- 3. Y que nadie cree objetos sueltos en el esquema public.
revoke create on schema public from public;

select 'aislamiento aplicado sobre atlas' as resultado;
