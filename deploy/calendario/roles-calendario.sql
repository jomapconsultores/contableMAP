-- Aislamiento de la base `calendario` dentro del PostgreSQL compartido.
--
...
--
-- Se ejecuta conectado a la base `calendario`.

-- 1. Rol de conexión propio de Calendario.
--    PostgREST se conecta con él y luego cambia a anon o service_role según
--    el token que traiga la petición.
do $$
begin
  if not exists (select 1 from pg_roles where rolname = 'authenticator_ca') then
    create role authenticator_ca with login noinherit;
  end if;
end $$;

alter role authenticator_ca with password :'clave';
...
grant anon, authenticated, service_role to authenticator_ca;
...
--    contra `calendario`.
revoke connect on database calendario from public;
grant  connect on database calendario to authenticator_ca, postgres;

-- 3. Y que nadie cree objetos sueltos en el esquema public.
revoke create on schema public from public;

select 'aislamiento aplicado sobre calendario' as resultado;
